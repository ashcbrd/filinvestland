...
function Client({ typeID, featured }: any) {
  ...
  const onMore = () => {
    ...
    Request()
      .get(`${process.env.NEXT_PUBLIC_CMS_URL}/api/futura-news?limit=6&sort=-Date&page=${page + 1}&${titleQuery}`)
      .then((response: any) => {
        setPage(page + 1);
  ...
  return (
    <div className="px-4 md:px-0">
      <NewsBanner featured={featured.FeaturedNews} newsType="News" title="News" />
      <div className="min-h-screen w-full bg-[#FFF8F8] pb-[117px] pt-[61px]">
        <div className="mx-auto w-full pb-[30px]">
          <BasicSearch isfor="News" />
        </div>
        {news.docs.length > 0 && (
          <section className="container">
            <p className="mx-[70px] py-0 px-[20px] sm:px-0 sm:mx-0 md: mx-[25px]">
              Showing {news.docs.length ?? "0"} of {news.totalDocs ?? "0"} News
            </p>
          </section>
        )}
        {news.docs.length > 0 ? (
          <NewsSection news={news.docs} />
        ) : (
