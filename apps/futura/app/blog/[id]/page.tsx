...
async function getData(id: any) {
  try {
    ...
    const newsQuery = await fetch(
      `${process.env.CMS_URL}/api/futura-news${stringifiedQuery}`,
      {
        cache: "no-store",
        method: "GET",
        headers: {
...
const Index = async ({ params }: any) => {
  ...
  return (
    <>
      <Banner imageUrl={page?.coverImage?.url} title={page?.title} description={page?.shortDescription} date={page?.Date} />

      <div className="container mx-auto !mb-20 !mt-20 flex min-h-screen w-full flex-col gap-x-8">
        <div className="flex flex-1 flex-col justify-center gap-4">{serializeRichText(page.content, true)}</div>
        <div>
          <p className="mb-7 mt-10 font-quicksand text-[22px] font-bold">Featured Communities</p>
