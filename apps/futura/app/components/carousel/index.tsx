...
function Carousel({ title, description }: Props) {
  ...
  return (
    <div className="container mx-auto">
      <div className="h-[800px] mx-auto relative ">
        {slides.map((slide, index) => (
          <div
            key={index}
            className={`absolute top-0 left-0 h-full transition-opacity ${
          ...
          >
            <img
              src={slide.image}
              alt={`Slide ${index + 1}`}
              className="w-full h-full object-cover h-800 rounded-[20px]"
            />
