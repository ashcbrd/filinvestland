...
import Table from "@/components/pages/investor-relations/disclosures/Table";
...
import Pagination from "@/components/pages/investor-relations/disclosures/Pagination";
...
export default function DisclosuresBlock({ content, searchParams, slug }: any) {
  ...
  useEffect(() => {
    if (dataCategories) {
      const disclosure = dataCategories.docs.filter((e) => e.slug === slug);
      setDisclosuresFilter(disclosure?.[0]?.title);
    }
  }, [dataCategories]);
          ...
          <div className="w-full flex-1">
            ...
            <MainDropdown
              ...
              action={(type: string) => {
                if (type === "All") {
                  navigate.push(
                    `/disclosures/${slug}?page=${page}&year=${yearFilter}`
                  );
                } else {
                  const disclosure = dataCategories?.docs.filter(
                    (e) => e.title === type
                  );
                  navigate.push(
                    `/disclosures/${disclosure?.[0].slug}?page=${page}&year=${yearFilter}`
                  );
                }
              }}
            />
