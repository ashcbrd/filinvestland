...
export const metadata = {
  ...
  description:
    "Being able to help everyone expand their knowledge on Philippine real estate is our goal. Click here!",
};
...
async function getData(keyword: any) {
  ...
  const featured = fetch(
    `${process.env.CMS_URL}/api/globals/aspire-featured-news`,
    { cache: "no-store" }
  );
  const news = fetch(
    `${process.env.CMS_URL}/api/aspire-news?limit=4&sort=-Date${
      query ? `&${query}` : ""
    }`,
    { cache: "no-store" }
  );
